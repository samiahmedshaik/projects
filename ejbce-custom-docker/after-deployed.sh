...
keystore_test_and_rewrite() {
    ...
    local keyStoreKeypasswd="${keyStoreBaseName}.keypasswd"
    local targetKeyStore="$2"
    local targetKeyStoreProtection="$3"
    if [ -f "${keyStoreBaseName}.keypasspwd" ] ; then
        log "WARN" "'${keyStoreBaseName}.keypasspwd' should be renamed to '${keyStoreBaseName}.keypasswd'."
        keyStoreKeypasswd="${keyStoreBaseName}.keypasspwd";
    fi
    ...
    read -r storepasswd<"$keyStoreStorepasswd"
    if [ -f "$keyStoreKeypasswd" ] ; then
        read -r keypasspwd<"$keyStoreKeypasswd"
    else
    ...
    java_keytool -keypasswd -keystore "$targetKeyStore" -storepass "$targetKeyStoreProtection" -keypass "$keypasspwd" -new "$targetKeyStoreProtection" -alias "$keyAlias" \
        | log "INFO"
    if [ $? -ne 0 ] ; then
        log "WARN" "The keystore ${keyStoreJks} was present, but could not be used with the provided protection."
        return 1
    fi
    return 0
}
...
keystore_generate_self_signed() {
    ...
    java_keytool -genkey -alias "${instanceHostname}" \
        ...
        -ext "SAN=dns:${instanceHostname}" -dname "O=${ORG_NAME},UID=${uniqueUid},CN=${instanceHostname}" \
        || return 1 | log "INFO"
...
# Does the application require any customization before proceeding with TLS etc
if [ -f ${baseDir}/bin/internal/after-deployed-app-customize.sh ] ; then
    . ${baseDir}/bin/internal/after-deployed-app-customize.sh "${baseDir}" "${tempDir}"
fi
    ...
    if [ $anyFailure = 0 ] ; then
        ...
        if [ -d $baseDir/secrets/external/tls/cas/ ] ; then
            for f in $baseDir/secrets/external/tls/cas/*.crt ; do
                if [ -f "$f" ] ; then
                    java_keytool -import -noprompt -file $f -alias ca-$(basename $f) \
                        -keystore ${tempDir}/truststore.jks \
                        -storepass "$trustStorePassword"
                else
                    log "WARN" "'$f' exists but is not a file and will be ignored. (Does the file exist outside the container?)"
                fi
            done
        fi
